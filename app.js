...
function loadScene(name){
  ...
  if(name==='voie_insertion'){
    const sand='#d4c4a0';
    const rW=46;
    // 2 voies sens gauche→droite : y1 (rapide, haut), y2 (lente, bas)
    const y1=cy-rW, y2=cy;
    // Accotements
    drawings.push({t:'rect',x:0,y:0,         w:W,h:y1-6,     color:sand,fill:sand,sz:0});
    drawings.push({t:'rect',x:0,y:y2+rW+6,   w:W,h:H-y2-rW-6,color:sand,fill:sand,sz:0});
    // 2 voies principales
    drawings.push({t:'road',x1:0,y1:y1,x2:W,y2:y1,lanes:1});
    drawings.push({t:'road',x1:0,y1:y2,x2:W,y2:y2,lanes:1});
    // Voie d'insertion : part du coin bas-gauche, rejoint y2 au 60% de l'écran
    // Elle longe le bord inférieur puis remonte doucement
    const joinX = W*0.62;
    drawings.push({t:'roadcurve',x1:0,y1:y2+rW*1.4,x2:joinX,y2:y2,lanes:1});
    // Zone hachurée triangulaire entre bas de y2 et courbe d'insertion
    drawings.push({t:'scene_hatch',x1:10,y1:y2+rW,x2:joinX*0.85,y2:y2+rW*1.3});
    // Panneau triangle rouge CÉDEZ à l'entrée (côté gauche)
    elements.push({special:'sign_cede',label:'',x:joinX*0.3,y:y2+rW*0.85,sz:44,rot:0});
    // Voitures (vert en haut rapide, jaune en insertion, bleu en voie lente)
    elements.push({icon:'🚗',label:'',x:W*0.10,y:y1+rW*0.5,sz:36,rot:0});
    elements.push({icon:'🚙',label:'',x:W*0.58,y:y1+rW*0.5,sz:36,rot:0});
    elements.push({icon:'🚗',label:'',x:W*0.25,y:y2+rW*0.6,sz:36,rot:0});
  ...
  if(name==='voie_deceleration'){
    ...
    drawings.push({t:'road',x1:0,y1:y1,x2:W,y2:y1,lanes:1});
    drawings.push({t:'road',x1:0,y1:y2,x2:W,y2:y2,lanes:1});
    // Voie de décélération : se détache de y2 au 40% de l'écran, file vers bas-droit
    const splitX = W*0.42;
    drawings.push({t:'roadcurve',x1:splitX,y1:y2,x2:W,y2:y2+rW*1.4,lanes:1});
    // Zone hachurée triangulaire entre voie principale et voie de décélération
    drawings.push({t:'scene_hatch',x1:splitX+W*0.08,y1:y2+rW,x2:W-10,y2:y2+rW*1.3});
    // Flèches vertes (comme sur l'image de référence)
    drawings.push({t:'arrow',x1:W*0.55,y1:y1+rW*0.5,x2:W*0.88,y2:y1+rW*0.5,color:'#30b94a',sz:5,dash:false});
    drawings.push({t:'arrow',x1:W*0.55,y1:y2+rW*0.5,x2:W*0.88,y2:y2+rW*0.5,color:'#30b94a',sz:5,dash:false});
    // Voitures
    elements.push({icon:'🚗',label:'',x:W*0.08,y:y1+rW*0.5,sz:36,rot:0});
    elements.push({icon:'🚙',label:'',x:W*0.55,y:y1+rW*0.5,sz:36,rot:0});
    elements.push({icon:'🚗',label:'',x:W*0.25,y:y2+rW*0.5,sz:36,rot:0});
